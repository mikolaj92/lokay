...
def test_needs_evidence_runs_catalog_then_one_agent(tmp_path):
    pytest.importorskip("fala")
    root=Path(__file__).resolve().parents[1]
    catalog_sentinel=tmp_path/"catalog"
    ...
    effector.write_text(
        ...
        "if a=='review_evidence_catalog': v['route']='agent'; Path("+repr(str(catalog_sentinel))+").write_text('ran')\n"
    ...
    assert run.returncode==0,run.stderr
    result=json.loads(run.stdout.strip().splitlines()[-1]); statuses={k:v['status'] for k,v in result['effector_results'].items()}
    assert statuses['review_evidence_catalog']=='succeeded'
    assert 'collect_review_diff_tail' not in statuses
    assert 'verify_review_evidence_sha' not in statuses
    assert catalog_sentinel.exists() and evidence_agent_sentinel.exists() and merge_sentinel.exists()


def test_red_checks_run_repair_node_not_review_or_merge(tmp_path):
    pytest.importorskip("fala")
    root = Path(__file__).resolve().parents[1]
    review_sentinel = tmp_path / "review-ran"
    merge_sentinel = tmp_path / "merge-ran"
    ...
    path = next(
        ...
        if item["id"] == "pr_triage"
    )
    commands = {item["id"]: [sys.executable, str(effector)] for item in path["effectors"]}
    script = (
        "import fala,json,sys; print(json.dumps(fala.host_run_package("
        "db_path=sys.argv[1],package_path=sys.argv[2],path_id='pr_triage',"
        "run_id='red-checks',command_overrides=json.loads(sys.argv[3]),max_ticks=32)))"
    )
    env = os.environ.copy()
    for key in (
        "LOKAY_ROOT",
    ...
    env.setdefault("PYTHONPATH", "")
    run = subprocess.run(
        [sys.executable, "-c", script, str(tmp_path / "db.sqlite"), str(package), json.dumps(commands)],
        cwd=root,
        env=env,
        capture_output=True,
        text=True,
    ...
    assert not review_sentinel.exists()
    assert not merge_sentinel.exists()
