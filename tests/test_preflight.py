...
def test_live_review_preflight_requires_a_non_empty_ocr_credential(tmp_path, monkeypatch):
    cfg = _config(tmp_path)
    cfg.write_text(
        cfg.read_text()
...
def test_live_review_preflight_accepts_ocr_credential_without_exposing_it(tmp_path, monkeypatch):
    ...
    finding = next(item for item in result["findings"] if item["name"] == "pr_review_credential")
    assert finding == {"name": "pr_review_credential", "ok": True, "code": "ok", "detail": "ok", "repaired": False}
    assert "test-review-credential" not in json.dumps(result)
...
def test_leftover_incident_import_error_reports_planned_not_live(tmp_path, monkeypatch):
    ...
    assert dry["planned"] is True
    src = Path(__file__).resolve().parents[1] / "src" / "lokay" / "preflight.py"
    assert "Leftover-incident ImportError reports planned=not live." in src.read_text(
        encoding="utf-8"
    )


def test_leftover_incident_import_error_reports_probe_failed(tmp_path, monkeypatch):
    ...
    assert out["applied"] is False
    src = Path(__file__).resolve().parents[1] / "src" / "lokay" / "preflight.py"
    assert "Leftover-incident ImportError reports probe_failed." in src.read_text(
        encoding="utf-8"
    )


def test_leftover_incident_empty_name_is_not_applied(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(preflight.subprocess, "run", boom)
    out = preflight._close_resolved_incidents("", loaded)
    assert out["applied"] is False
    assert out["closed"] == []
    src = Path(__file__).resolve().parents[1] / "src" / "lokay" / "preflight.py"
    assert "Leftover-incident empty name is not applied." in src.read_text(
        encoding="utf-8"
    )
...
def test_leftover_incident_empty_name_reports_probe_failed(tmp_path, monkeypatch):
    ...
    assert out["applied"] is False
    src = Path(__file__).resolve().parents[1] / "src" / "lokay" / "preflight.py"
    assert "Leftover-incident empty name reports probe_failed." in src.read_text(
        encoding="utf-8"
    )
