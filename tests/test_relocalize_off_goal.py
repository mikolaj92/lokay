...
def test_relocalize_restores_hot_repos_test_when_issue_does_not_name_it(
    monkeypatch, tmp_path: Path, capsys
):
    (tmp_path / ".lokay").mkdir()
    path = "tests/test_hot_repos.py"
    (tmp_path / ".lokay/localize.json").write_text(
...
def test_relocalize_keeps_hot_repos_test_when_issue_names_it(
    monkeypatch, tmp_path: Path, capsys
):
    (tmp_path / ".lokay").mkdir()
    path = "tests/test_hot_repos.py"
