...
def apply_intake(
    ...
    *,
    ...
    Still-open issues are never closed here. Already-closed is the only
    exception (same refuse as apply_issue_close).
    """
    ...
    if decision.comment:
        comment_issue(runner, repo, issue_number, decision.comment, live=True)
    if decision.close and str(issue.state or "OPEN").strip().upper() == "CLOSED":
