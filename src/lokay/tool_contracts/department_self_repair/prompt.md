...
Working factories retry the coding session or re-run CI. They do not treat leftover, occupancy, or idle as a reason to rewrite the orchestrator. You must not either.
...
Work:
1. Confirm a factory stall before touching `mikolaj92/lokay` main. These are **not** stalls: leftover skip, occupancy K=1, idle, pass_ceiling, waiting, empty survey, `health=hosted`.
