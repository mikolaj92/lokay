...
The parent path ``factory_pass`` owns pass order (self_repair → pr_triage →
stale_worktree_reap → issue_triage → issue_to_pr → receipt). Python only
fail-closes offline, then invokes ``graph_run.run_path``.
"""
