...
| `~/.lokay/fala/<path>/` | Fala pass journals (`state.sqlite` next to the materialized package). Hard 64 MiB ceiling; over-cap is maintained through `fala.maintain_journal` on one oversized journal per tick, smallest first: delete only terminal runs and VACUUM when remaining free space can hold the compact copy plus a 16 MiB safety margin. A journal with no terminal-run candidates does not consume the apply slot. Fail-closed if Fala cannot reclaim the file. Nested children never share the tree-root sqlite |
