...
| Atom | One job |
| --- | --- |
| `host_ff` | mill host fetch + ff-only onto origin/main; refuse if dirty / host catalog skip-worktree would overwrite. Product `config.yaml` follows origin/main. Launchd caretaker skips the mill pass when `mill.lock` is held. After ff moves HEAD, mill-daemon reinstalls and starts `lokay-daemon` in the same tick; live i2pr is not killed. Crash KeepAlive (`SuccessfulExit=false`) restarts a failed tick without waiting StartInterval; idle 0 stays on the 60s heartbeat |
