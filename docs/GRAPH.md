...
| `factory_begin` | preflight + pass workspace + budgets; refuse when in-cycle `host_ff` just updated **or** `LOKAY_PROCESS_HEAD` drifted (restart, do not mill on the previous import). Auth probe must not treat a GitHub `/user` 503 as a missing token. Healthy first host check is not rerun (`gh api user` / ast.parse). Repair still reruns `_check`. Healthy preflight closes leftover `<!-- lokay-preflight:… -->` tickets. After an empty leftover-incident probe, skip that GitHub list for 300s without refreshing the stamp. Fresh leftover-incident skip is not applied. Empty leftover-incident host is not applied. Idle leftover-incident skip outlives leftover-probe. Hosted factory_pass stays at 300s. Pytest must not skip leftover-incident GitHub lists using the mill stamp. |
