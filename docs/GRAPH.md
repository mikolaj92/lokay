...
| `queue_conflict` | contradiction gate before implement (queue hygiene). After `needs_human` / skip / close parks the selected issue, `advance_implementation_selection` re-reduces the catalog so the next implementable product row keeps `clean_repos`. One parked product issue must not empty the K=1 slot. |
