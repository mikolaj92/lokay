...
| `survey_ready` | list implementable open catalog issues (human stops exclude; `work:ready` / `ai:ready` are optional ledger traces, not a gate); skip those covered by open AI PRs. Missing `state` is still OPEN work; only explicit CLOSED parks. After a complete empty mill survey, skip GitHub lists for 120s without refreshing the stamp. |
