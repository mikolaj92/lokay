...
The authored listing order is harvest, then `host_ff`, then the host gate,
then the five departments, then `record_pass`, then `factory_pass_terminal`,
then the sibling `reap_stale_worktrees`. Reap conducts from
`factory_begin_host_gate` and `factory_begin`; its listing position must not
turn it into a dependency of product or the terminal.
...
| `factory_begin_host_gate` | Succeeds with `route=begin`, `route=restart`, or `route=blocked` (`health=host_behind`). Restart means host-ff moved HEAD under this process. `blocked` is a failed or missing host sync, not a restart. Never `ok=false`: a failed gate still unblocks product children in Fala. |
...
| lokay Fala journals | every live `state.sqlite` under `~/.lokay/fala/<path>/` is maintained through `fala.maintain_journal` at a 64 MiB ceiling; one oversized journal per tick, smallest first, deletes only terminal runs and VACUUMs when remaining free space can hold the compact copy plus a 16 MiB safety margin. A journal with no terminal-run candidates does not consume the apply slot. Heartbeat `created` leftovers are not finalized by maintenance. Self-repair incomplete runs keep their original status. `daemon_entry` / `daemon_cycle` / `factory_pass` use a fresh wrapper sqlite per tick; they do not reopen the shared lokay journals. Each host file contains only the requested `path_id`, not the whole authored package. Recovery stays on `state.jsonl`. Nested children never share the tree-root sqlite or overwrite a sibling materialized package. Over-cap is fail-closed if Fala cannot maintain the file |
| lokay activity | each live lokay organ atom writes `activity.json` beside `state.jsonl` (`path`, `atom`, `work_id`, `transitions`, `last_activity_at`). `daemon_entry` resets the checkpoint at the start of a tick so `transitions` do not accumulate across heartbeats. Ceiling receipts resume from that file. `status_snapshot` and `live=false` organs never write it. Status stays read-only. A missing file is `ceiling_stalled`, not a crash. CEO/status `health=progress` = DoD only (inflight\|merge\|new_pr), not Fala transitions (#1042) |
...
Parent sieve leftover (department, not this child):

```text
list_pr_sieve
  → select_pr_sieve     leftover_prs from last-pass; consume skip without merge
    → reconcile / run     one PR per pass (K=1)
