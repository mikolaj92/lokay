...
| `survey_prs` | list open AI PRs for all repos (full page; cap is survey_error). After a complete empty mill survey, skip GitHub lists for 120s without refreshing the stamp. A live mill with that fresh stamp and idle last-pass also skips hosting `factory_pass` until the stamp expires. |
