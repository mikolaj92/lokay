...
| mill Fala journals | every live `state.sqlite` under `~/.lokay/fala/<path>/` rotates at a 64 MiB ceiling; recovery stays on `state.jsonl`. Nested children never share the tree-root sqlite or overwrite a sibling materialized package. Over-cap is fail-closed if the file cannot be cut |
