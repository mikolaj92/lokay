...
| mill activity | each live mill organ atom writes `activity.json` beside `state.jsonl` (`path`, `atom`, `work_id`, `transitions`). Ceiling receipts resume from that file. `status_snapshot` and `live=false` organs never write it. Status stays read-only. A missing file is `ceiling_stalled`, not a crash |
