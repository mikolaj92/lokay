...
A classified complete reject (`ocr_contract_rejected`) / non-review skip on
the current SHA is a consumed row. The receipt leftover is the rest of the
open lokay PRs. Incomplete OCR (`ocr_timed_out`) KEEP the same identity so
the next pass retries that SHA until a complete JSON exists. Pending checks
KEEP the row. A new head SHA is a new identity and may be reviewed again.
This is queue hygiene, not a parallel scheduler and not a human-approval gate.
