...
3. **Implement ready**: Fala/`issue_to_pr` → agent → commit/push → PR. Worktree is **reset to `origin/main`** (`lokay-worktree-add --reset-base`) so re-implement after a closed conflict does not reuse a stale tip. Skip ready issues that already have an open `ai/fix/*` PR (owned by PR triage). Stuck failures → ledger → `ai:blocked`. Live ready work with `executor.enabled: false` is a **stall** (agent never runs).
