...
2. **Per-repo PR-first**: PR close-out (conflict / repair / triage / waiting) is
   ...
   (`ai:needs-review`) never freeze unrelated repos. Safety: never open a second
   `ai/fix/*` PR in a repo that already has an open AI PR.
3. **Inbox triage + intake** (per repo, when that repo has no actionable open AI
   PR and its PR survey succeeded): undecided issues → triage rules, then
   deterministic **intake** → `ai:ready` | `ai:needs-feedback` | CLOSE/OOS.
   Path: `issue_triage` (`get_issue → triage_issue → intake_issue`). Intake is a
   deterministic frame (shape/playbook fitness, superseded, already-satisfied,
   ambiguity) — not an agent-orchestrator. Fail closed: a failed PR survey for
   a repo refuses triage mutations **in that repo only**.
...
5. **Implement ready (parallel K)**: up to **K** `issue_to_pr` runs per factory
   ...
   implement (demote/close instead). Then configured executor → commit/push →
