...
3. **Implement ready**: Fala `issue_to_pr` → agent → commit/push → PR. Stuck failures → ledger → `ai:blocked` so other issues still mill.
4. **PR triage**: checks; red → `pr_repair`; green + `merge.enabled` → merge + close linked issue.
5. **Health**
