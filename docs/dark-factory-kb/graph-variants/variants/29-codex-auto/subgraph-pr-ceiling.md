...
```mermaid
flowchart TD
  A([enter: branch codex/issue-N]) --> B[DET: gh pr create Closes N]
  ...
  H -->|no / high| E
  G --> I([leave: merged])
  E -->|review comments / re-label| J([re-enter: label-start / leaf])
...
- **Coder ≠ merge.** Codex CLI (i GHA harness) kończą na PR. Gałka Off\|Classify\|Always jak w ready-for-agent; start **Off**.
- **Closes #N.** DET body PR linkuje issue; branch już `codex/issue-N`.
...
- **Fix loop.** Komentarz / re-label / `@codex` wraca do label-start → det-harness → leaf; nie merge z liścia.
