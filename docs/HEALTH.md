# Lokay health without watching GitHub
...
| Field | Meaning |
| --- | --- |
| `lokay_ready` | Config can lokay live (mode/executor/merge gates) |
...
| `human_residuals` | Compact needs-feedback / needs-review count (not a lokay brake) |
...
Only the lokay-lock owner mints the parent run capability. A detached
...
Daemon logs remain under `~/.lokay/logs/lokay-latest.log` (see
`scripts/lokay-service.sh`). The receipt is the structured signal; logs are
...
envelope stays in `lokay-*.log`, which is size-capped and rotated. Bound is
...
`host_ff` skips when `lokay.lock` is held; `LOKAY_PROCESS_HEAD` still
...
`ok=false` on status means **not working** (work remains but lokay not live-ready,
...
`merge_disabled`). Green checks with `merge.enabled` false are `waiting` (lokay
...
mailbox residual (`waiting` / `human_residuals`). They must not count as lokay
...
A clean repo with leftover ready is still a stall if the lokay made no progress.
...
**Recovery boundary:** `waiting` / `repairing` (and other soft lokay health) must
